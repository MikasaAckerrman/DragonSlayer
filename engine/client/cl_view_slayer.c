...
void Slayer_OnDeathMsg( const byte *pbuf, int iSize )
{
	...
	if( killer == cl.playernum + 1 && killer != victim && killer != 0 )
	{
		...
		// Slayer3D: synthetic "you dealt damage" event on every confirmed
		// local kill — stock servers don't broadcast per-shot dealt-damage,
		// so the crosshair damage indicator relies on this fallback.
